...
local CreateDrawing = function(DrawingType, Properties, ...)
...
do
    ...
    local function ComputeBoundingBoxFromParts(Parts, Min, Max)
        for _, Part in IPairs(Parts) do
            if Part:IsA("BasePart") then
                ...
                Min = Vector3New(MathMin(Min.X, P.X - HalfSize.X), MathMin(Min.Y, P.Y - HalfSize.Y), MathMin(Min.Z, P.Z - HalfSize.Z))
                Max = Vector3New(MathMax(Max.X, P.X + HalfSize.X), MathMax(Max.Y, P.Y + HalfSize.Y), MathMax(Max.Z, P.Z + HalfSize.Z))
            end
        end
        local Size = Max - Min
    ...
    function PlayerEsp:HumanoidHealthChanged()
        ...
        self.Drawings.HealthBar.Color = ColorRed:Lerp(ColorGreen, HealthPercentage)
    end

    function PlayerEsp:SetupHumanoid(Humanoid, FirstTime)
        self:HumanoidHealthChanged()
        self.Connections[#self.Connections + 1] = Humanoid:GetPropertyChangedSignal("Health"):Connect(function() self:HumanoidHealthChanged() end)
        if FirstTime then
    ...
    function PlayerEsp:RenderFlags(Center2D, Offset, FlagsSettings, BottomYOffset)
        ...
        for i = 1, Count do
            ...
            if Mode == "always" then
                local TrueColor = (Item and Item.ColorTrue) or ColorGreen
                local FalseColor = (Item and Item.ColorFalse) or ColorRed
    ...
    function EntityEsp:CreateDrawingCache()
        ...
        local Drawings = {
            ...
            Name = CreateDrawing("Text", {
                ...
                Outline = true,
                OutlineColor = ColorBlack,
                Color = self.Color or ColorWhite,
                Transparency = 1,
                Size = Cfg.TextSize,
                Text = self.Name or "",
                Font = Cfg.Font,
    ...
    function NpcEsp:Loop(Settings, DistanceOverride)
        ...
        if Settings.Healthbar then
            ...
            HealthBar.Position = HealthPosition
            HealthBar.Size = HealthSize
            HealthBar.Color = ColorRed:Lerp(ColorGreen, HealthPercentage)
        else
            self.Drawings.HealthBar.Visible = false
            self.Drawings.HealthBackground.Visible = false
        end
    ...
    function NpcEsp:RenderFlags(Center2D, Offset, FlagsSettings, BottomYOffset)
        local FlagTexts = self.Drawings.FlagTexts
        for i = 1, #FlagTexts do
            FlagTexts[i].Visible = false
        end
    ...
    function EspLibrary:Unload()
        ...
        for _, NpcEspInstance in Pairs(NpcEsp.NpcCache) do
            NpcEsp.Remove(NpcEspInstance.Model)
        end
